...
function EditPetForm() {
    ...
    useEffect(()=>{
        axios.get(`https://olayinka-pawster-capstone-project.onrender.com/pets/${petId}`).then(
            (response) => {
    ...
    const updatePetInfo = async (updatedPet, petId) => {
      await axios.post(`https://olayinka-pawster-capstone-project.onrender.com/pets/${petId}`, updatedPet).then((res) => {
        const data = res.data
        console.log(data)
       navigate(`/shelter/${id}`);
      }, 
